...
DECLARE
  ...
  v_max_squad INT := 15;
BEGIN
  -- Compute the most recent Thursday 3:00 PM Pacific Time.
  -- DOW: 0=Sun,1=Mon,...,4=Thu,6=Sat. Days since Thursday = (dow+3)%7.
  v_week_start := (
    date_trunc('day',
      (NOW() AT TIME ZONE 'America/Los_Angeles')
      - ((EXTRACT(DOW FROM NOW() AT TIME ZONE 'America/Los_Angeles')::int + 3) % 7 || ' days')::interval
    ) + INTERVAL '15 hours'
  ...
  IF v_team.fa_window_used_at IS NOT NULL AND v_team.fa_window_used_at >= v_week_start THEN
    RETURN jsonb_build_object('error', 'Free agent window already used this week. Resets Thursday at 3:00 PM PT.');
  END IF;
