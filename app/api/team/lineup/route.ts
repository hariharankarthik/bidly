...
export async function POST(req: NextRequest) {
  ...
  const cfg = getSportConfig(room.sport_id);
  const xiSize = cfg?.lineup?.xiSize ?? 11;
  const maxOverseasInXi = cfg?.lineup?.maxOverseasInXi ?? null;

  if (xi.length > xiSize) {
    return NextResponse.json({ error: `Starting XI can have at most ${xiSize} players` }, { status: 400 });
  }
  ...
  if (xi.length > 0) {
    if (squad.size < xiSize) {
      return NextResponse.json(
        { error: `You need at least ${xiSize} players on your squad before you can set a Starting XI` },
        { status: 400 },
      );
    }
    if (xi.length !== xiSize) {
      return NextResponse.json({ error: `Starting XI must have exactly ${xiSize} players` }, { status: 400 });
    }
  }
  ...
  if (maxOverseasInXi != null && xi.length > 0) {
    ...
    if (overseas > maxOverseasInXi) {
      return NextResponse.json(
        { error: `Starting XI can include at most ${maxOverseasInXi} overseas players` },
        { status: 400 },
      );
    }
  }
