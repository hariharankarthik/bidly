...
export async function POST(req: NextRequest) {
  ...
  if (league.league_kind !== "private") {
    return NextResponse.json({ error: "Only private leagues can be deleted here" }, { status: 400 });
  }
  if (league.host_id !== user.id) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }

  const { error: dErr } = await supabase.from("fantasy_leagues").delete().eq("id", league_id);
